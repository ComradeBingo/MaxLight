(function () {
    ...
    function getChatInfo(chatElement) {
        try {
            ...
            // Пропускаем системные сообщения "печатает" 
            var trimmedMessage = message.trim().toLowerCase();
            if (trimmedMessage === 'печатает') {
                return null;
            }
    ...
    function processNewChat(chatElement) {
        try {
            ...
            // НЕ ВЫЗЫВАЕМ sendUnreadCount() — это может прочитать сообщение!
    ...
    function setupObserver() {
        ...
        observer = new MutationObserver(function (mutations) {
            ...
            mutations.forEach(function (mutation) {
                    ...
                    mutation.addedNodes.forEach(function (node) {
                        if (node.nodeType === 1) {
                            ...
                            if (node.querySelector && node.querySelector('.badgeIcon')) {
                                console.log('🔔 Обнаружен новый бейдж');
                                needsRecount = true;
                            }
                        }
                    });
                ...
                // Удаление узлов (бейдж мог исчезнуть)
                if (mutation.removedNodes && mutation.removedNodes.length > 0) {
                    mutation.removedNodes.forEach(function (node) {
                        if (node.nodeType === 1) {
                            if ((node.classList && node.classList.contains('badgeIcon')) ||
                                (node.querySelector && node.querySelector('.badgeIcon'))) {
                ...
                // Изменение атрибутов (бейдж мог появиться/исчезнуть)
                if (mutation.type === 'attributes' && mutation.target) {
                    var target = mutation.target;
                    if (target.classList && target.classList.contains('badgeIcon')) {
                        console.log('🔔 Изменился бейдж');
                        needsRecount = true;
            ...
            // Пересчитываем счетчик если были изменения бейджей
            if (needsRecount) {
                setTimeout(function () {
                    if (typeof sendUnreadCount === 'function') {
                        sendUnreadCount();
                    }
                }, 500);
            }
    ...
    function sendUnreadCount() {
        ...
        for (var i = 0; i < chats.length; i++) {
            if (hasUnreadMessage(chats[i])) {
                unreadCount++; // Считаем чаты (для счетчика в трее)

                // Пытаемся посчитать реальные сообщения
                var badge = chats[i].querySelector('.badgeIcon.badgeIcon--20.badgeIcon--themed-regular');
                if (badge && badge.textContent) {
                    var num = parseInt(badge.textContent.trim());
                    if (!isNaN(num)) unreadMessages += num;
                } else {
                    unreadMessages++; // Если бейдж без числа — минимум 1
                }
            }
        }
        ...
        try {
            window.chrome.webview.postMessage(JSON.stringify({
                type: 'unread_count',
                count: unreadMessages > 0 ? unreadMessages : unreadCount
            }));
            console.log('📊 Счетчик отправлен: ' + unreadMessages + ' сообщений, ' + unreadCount + ' чатов');
        } catch (e) {
            console.log('❌ Ошибка отправки счетчика:', e);
        }
    }
