...
/// <summary>
/// Управляет модификациями HTML/JS страницы в WebView2 (WPF версия)
/// </summary>
public class PageModifier : IDisposable
{
    private readonly WebView2 _webView;
    private readonly List<IPageModification> _modifications = new();
    ...
    private void OnCoreWebView2InitializationCompleted(object? sender, CoreWebView2InitializationCompletedEventArgs e)
    {
        ...
        {
            // НЕ добавляем NewWindowRequested здесь, так как он уже обрабатывается в WebView2Handler
    ...
    public async Task InitializeModificationsAsync()
    {
        ...
        foreach (var modification in _modifications)
        {
            ...
            {
                Debug.WriteLine($"[PageModifier] Ошибка инициализации {modification.Name}: {ex.Message}");
            }
        }

        _isInitialized = true;
        Debug.WriteLine($"[PageModifier] Инициализировано {_modifications.Count} модификаций");
    }
    ...
    public async Task ApplyModificationsOnNavigationAsync()
    {
        ...
        foreach (var modification in _modifications)
        {
            ...
            {
                Debug.WriteLine($"[PageModifier] Ошибка применения {modification.Name}: {ex.Message}");
            }
        }
...
(function() {
    ...
    window.__maxlight_link_handler_installed = true;
        ...
        {
            Debug.WriteLine($"[PageModifier] Ошибка перехвата ссылок: {ex.Message}");
        }
    }

    public void AddModification(IPageModification modification) => _modifications.Add(modification);
...
// ==========================================
// ИНТЕРФЕЙС И РЕАЛИЗАЦИИ МОДИФИКАЦИЙ
// ==========================================
...
{
    public string Name => "RemoveStyleAttribute";
    public string Description => "Удаляет атрибут style у указанного элемента";
...
public class RemoveElementModification : IPageModification
{
    ...
    public string Description => "Удаляет элемент с указанным селектором";
    public bool IsEnabled { get; set; } = true;
...
public class CustomScriptModification : IPageModification
{
    ...
    public string Description => "Выполняет пользовательский скрипт";
    public bool IsEnabled { get; set; } = true;
    ...
    {
        _script = script;
        Name = name ?? $"CustomScript_{Guid.NewGuid():N}";
    }
