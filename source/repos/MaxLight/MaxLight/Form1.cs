    ...
    public partial class Form1 : Form
    {
        ...
        private async Task InitializeWebViewAsync(string userDataFolder)
        {
            try
            {
                ...
                if (!hasAuth)
                {
                    System.Diagnostics.Debug.WriteLine("Токенов нет, включаем парсер для ожидания авторизации");
                    await InjectTokenInterceptor();
                    _tokenParserActive = true;
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine("Токены восстановлены, парсер токенов не включаем");
                ...
                webView.CoreWebView2.NavigationCompleted += async (sender, args) =>
                {
                    if (args.IsSuccess)
                    {
                        ...
                        if (_tokenParserActive)
                        {
                            await StopTokenParser();
                            System.Diagnostics.Debug.WriteLine("✓ Парсер токенов остановлен после загрузки страницы");
                        }
        ...
        private async Task InjectTokenInterceptor()
        {
            string interceptorScript = @"
                (function() {
                    ...
                    localStorage.setItem = function(key, value) {
                        ...
                        if (key === '__oneme_auth') {
                            try {
                                ...
                                hasProcessed = true;
                                console.log('[MaxLight] ✓ Токен перехвачен и отправлен, парсер остановлен');
                            } catch(e) {
                    ...
                    console.log('[MaxLight] 🎯 Перехватчик токенов установлен (режим ожидания авторизации)');
                })();
            ";

            await webView.CoreWebView2.AddScriptToExecuteOnDocumentCreatedAsync(interceptorScript);
        }
        ...
        private async Task ClearSavedAuthData()
        {
            try
            {
                using (var key = Registry.CurrentUser.OpenSubKey("SOFTWARE", true))
                {
                    if (key != null)
                    {
                        key.DeleteSubKeyTree("MaxLight", false);
                        System.Diagnostics.Debug.WriteLine("Ветка MaxLight полностью удалена из реестра");
                    }
                }
